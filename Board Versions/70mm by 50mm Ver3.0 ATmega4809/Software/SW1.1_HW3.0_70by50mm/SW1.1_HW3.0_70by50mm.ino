 ...
 * [Optional] Build your own UPDI programmer with this tutorial: https://github.com/ElTangas/jtag2updi (Hint: the 4k7 resistor is already included in the SolderReflowPCB)
 */
 ...
 *      Reset Pin->Reset
 *  2) Set programmer of choice. If you followed the tutorial above, use jtag2updi
 *  3) Set Port. If you followed the tutorial above, use arduino uno(COM x)
...
void main_menu(void);       //Display main menu
bool heat(byte);            //Apply heat curve
void cancelledPB(void);     //Cancel heating with button
void cancelledTimer(void);  //Cancel heating with timeout
void coolDown(void);        //Cool down after heating
void completed(void);       //Display complete message
float getTemp(void);        //Read temp from sensor
float getVolts(void);       //Reat input voltage
...
void setup() {
  ...
  upButton.attach(upsw,INPUT);
  upButton.setPressedState(LOW);
  upButton.interval(20);

  downButton.attach(dnsw,INPUT);
...
void coolDown() {
  ...
  while(!downButton.pressed() && !upButton.pressed() && t > 45.00) {
    upButton.update();
    downButton.update();
    display.clearDisplay();
    display.drawRoundRect( 22, 0, 84, 32, 2, SSD1306_WHITE );
